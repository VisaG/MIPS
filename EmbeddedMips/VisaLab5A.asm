...
# Inner loop subroutine called matrixCalLoop unrolled 2 times with three iterations of the unrolled loop
...
matrixCalLoop:				#Matrix Calculation
	...
	beq $t6, 3, matrixCalEnd
	
	lw $t3, 0($t1)			#$t3 =  matrixArray1[i]
	lw $t4, 4($t1)			#$t3 = matrixArray1[1+4] / next row element
	...
	add $s3, $t5, $t9		#$s3 = $t5 + $t9
	
	addi $t1, $t1, 8		#Increment row element (i = 8) (stall removed by adding instruction here) 
	addi $t2, $t2, 48		#Increment to next column element (j = 24) (stall removed by adding instruction here) 
	
	add $t7, $t7, $s3		#$t7 = $t7 + $s3($t5+$t9)
